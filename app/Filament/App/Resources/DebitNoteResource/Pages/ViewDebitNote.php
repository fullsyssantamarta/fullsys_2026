    ...
    protected function getHeaderActions(): array
    {
        return [
            ...
            Actions\Action::make('send_to_dian')
                ...
                ->modalHeading('Enviar Nota Débito a DIAN')
                ->modalDescription('¿Está seguro de enviar esta nota débito a la DIAN? Esta acción no se puede deshacer.')
                ->modalSubmitActionLabel('Sí, enviar')
                ->action(function () {
                    try {
                        $apidian = new ApidianService();
                        $response = $apidian->sendDebitNote($this->record->toApidianFormat());
                        ...
                        if ($response['success']) {
                            ...
                            Notification::make()
                                ->title('Nota Débito enviada exitosamente')
                                ->success()
                            ...
                            Notification::make()
                                ->title('Error al enviar Nota Débito')
                                ->danger()
